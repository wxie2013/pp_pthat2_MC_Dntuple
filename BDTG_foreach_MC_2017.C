...
#include  "weights/TMVAClassification_pp_3_100_BDTG_5_cutvariables_0_1_2_3_5_log_test_weights/TMVAClassification_BDTG.class.C"

#define MAX_XB       20000

using namespace std;

int BDTG_foreach_MC_2017(TString inputfile="", int pbpb=0, int ptMin=3, int ptMax=100, string mvatype="BDTG",int cut_number=5,TString name_variable="0_1_2_3_5")
{
	...
	TFile *inf = new TFile(inputfile, "update");
	...
	for(int i=0;i<t->GetEntries();i++)
	{
		...
		for(int j=0;j<Dsize;j++)
		{
			...
			{
				inputValues.clear();
        inputValues.push_back((log10(DlxyBS[j]/sqrt(DlxyBSErr[j]))));
